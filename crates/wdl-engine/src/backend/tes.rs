...
impl TaskExecutionBackend for TesBackend {
    fn constraints(
        ...
        let disks: IndexMap<String, i64> = disks(requirements, hints)?
            .into_iter()
        ...
        Ok(TaskExecutionConstraints {
            container: Some(
                container(requirements, self.config.task.container.as_deref()).into_owned(),
            ),
    ...
    fn spawn(
        ...
        async move {
            ...
            loop {
                ...
                let statuses = match self.inner.run(task, token.clone())?.await {
